...
#define IP_ADDRESS "10.29.31.153"
//#define IP_ADDRESS "10.29.31.153"
//#define IP_ADDRESS "192.168.2.100"
#define PORT "8232"
...
bool setupWiFi() {
  if(sendATCommand("AT+RST", SERIAL_WAIT_TIME)) {        //AT+RST > Reset
    String wifiConnectCmd = "AT+CWJAP_CUR=\"";               //AT+CWJAP="SSID","PASSWORD" > WiFi-Connect
    wifiConnectCmd += SSID;
    wifiConnectCmd += "\",\"";
...
void setup(){
  ...
  //AP+server
  sendATCommand("AT+CWMODE_CUR=1", SERIAL_WAIT_TIME);
  ...
  sendATCommand("AT+CIFSR", SERIAL_WAIT_TIME);
