...
//  Serial.println(wifi.getVersion().c_str());
  ...
  do{
    if (wifi.createTCP(IP_ADDRESS, PORT)) {
      Serial.println("create tcp success");
      flag = 1;
    } else {
...
void loop(void) {
  ...
  PWM_Mode();
  delay(400);    //ここで遅延をつくれる？？ 500ms安定する？ 100msだとちょい不安定？
  PWM_Mode2();
  delay(400);
  PWM_Mode3();
  delay(400);
...
//ピンの入出力設定⑤
void PWM_Mode_Setup5(){ 
  pinMode(URTRIG5,OUTPUT);                            
  digitalWrite(URTRIG5,HIGH);                         
  pinMode(URPWM5, INPUT);                             
  for(int i=0;i<4;i++){
    Serial.write(EnPwmCmd[i]);
    mySerial.write(EnPwmCmd[i]);
  } 
...
//センサ④情報の抽出
void PWM_Mode5(){                                    
  digitalWrite(URTRIG5, LOW);                       
  digitalWrite(URTRIG5, HIGH);
  unsigned long DistanceMeasured5 = pulseIn(URPWM5,LOW);
  //距離測定
  if(DistanceMeasured5==50000){                    
    Serial.print("Invalid");    
  } else {
    Distance5 = DistanceMeasured5 / 50;              
  }
  //距離の書き出し
  Serial.print("5=");
